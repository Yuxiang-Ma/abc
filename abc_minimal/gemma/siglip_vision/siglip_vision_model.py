...
class AveragePool2D(nn.Module):
  """Applies average pooling and reshaping."""
  def __init__(self, config):
    super().__init__()
    self.config = config
    # Calculate kernel size and stride dynamically
    # Input tokens: (image_size // patch_size)^2
    # Output tokens: encoding_sequence_length
    # We assume square grids.
    self.patches_per_dim = config.image_size // config.conv2d_patch_size
    self.output_grid_dim = int(config.encoding_sequence_length**0.5)
    if self.patches_per_dim % self.output_grid_dim == 0:
        self.kernel_size = self.patches_per_dim // self.output_grid_dim
        self.stride = self.kernel_size
    else:
        # Fallback to defaults if non-divisible (e.g. 4 for 896->256 tokens)
        self.kernel_size = 4
        self.stride = 4
  ...
  def forward(self, x):
      ...
      # If kernel_size is 1, just return x (reshape/flatten might be needed if format differs, but here input is BxLxC)
      if self.kernel_size == 1:
          return x

      # Bx(H*W)xC -> BxCxH*W -> BxCxHxW
...
class SiglipVisionModel(nn.Module):
  ...
  def forward(
    ...
    # SigLIP exit: average-pool the final encoder features.
